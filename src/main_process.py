...
def process_mask_image(anomaly_scores_df, mask_image_file, threshold, mask_image_folder):
    """
...
def generate_pics(base_paths,base_image_file, back_image_file, anomaly_scores_df, threshold=None, scale_factor=1):
    """
    ...
    output_folder = base_paths["output_root"] / f"{Path(base_image_file).stem}-{Path(back_image_file).stem}"
    aligned_data_path = base_paths["aligned_data_path"]#calibration data
    output_folder.mkdir(parents=True, exist_ok=True)

    # Determine threshold
    if threshold is None:
        threshold = pic_prepro.get_best_threshold(anomaly_scores_df, Path(base_image_file).stem, is_vertical)

    # Preprocess images
    base_image, base_array, base_image_stem = preprocess_image(base_image_folder, base_image_file)
    back_image, back_array, back_image_stem = preprocess_image(back_image_folder, back_image_file)
    mask_image, mask_array, mask_image_stem = process_mask_image(anomaly_scores_df, base_image_file, threshold, mask_image_folder)
    ...
    masked_diff_array = pic_pro.apply_mask_to_image(diff_array, mask_array)
    aligned_back_image, aligned_diff_image = pic_pro.align_image_arrays(
        back_array, back_image_dis_from_edge, masked_diff_array, base_image_dis_from_edge, is_vertical, False
    )#shift masked_diff_array
    
    # Scale and save results 
    aligned_diff_image = pic_pro.scale_image(aligned_diff_image, scale_factor)
    result_array = pic_pro.apply_difference_to_image(aligned_back_image, aligned_diff_image)
