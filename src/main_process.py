...
def generate_pics(base_image_file,back_image_file,anomaly_scores_df,threshold=None,scale_factor=1):
    ...
    mask_image_folder = Path(r"C:\Users\huang\work\dataset\calibration\data\20241217\mask")
    back_image_folder = Path(r"C:\Users\huang\work\dataset\calibration\data\20241217\normal")
    output_root = Path(r"C:\Users\huang\work\dataset\calibration\data\20241217\output")
    output_folder = output_root / f"{Path(base_image_file).stem}-{Path(back_image_file).stem}"
    aligned_data_path=Path(r"C:\Users\huang\work\dataset\calibration\data\merged_reference_data.csv")
    ...
    back_image=pic_prepro.resize_image(img=back_image,size=(512,512),save=False)#todo save display
    ...
    #process
    ###
    base_image_dis=pic_pro.get_distance(base_image_stem,aligned_data_path)
    back_image_dis=pic_pro.get_distance(back_image_stem,aligned_data_path)
    ...
    aligned_base_image, aligned_reference_image=pic_pro.align_images(back_array,back_image_dis,masked_diff_image_array,base_image_dis,is_vertical,False)
