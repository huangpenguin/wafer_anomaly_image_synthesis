...
def train(cfg: DictConfig) -> tuple[dict, dict]:
    if cfg.case_bin_clf.get("seed"):
        seed_everything(cfg.case_bin_clf.seed)
    if not Path(cfg.case_bin_clf.save_dir).exists():
        Path(cfg.case_bin_clf.save_dir).mkdir(parents=True)
    datamodule = SupervisedDataModule(
        image_dir=cfg.case_bin_clf.data_dir,
        image_size=cfg.case_bin_clf.data_size,
        batch_size=cfg.case_bin_clf.batch_size,
        num_workers=cfg.case_bin_clf.num_workers,
        aug_dict=cfg.case_bin_clf.aug_dict,
        split_num=cfg.case_bin_clf.split_num,
        split_seed=cfg.case_bin_clf.seed,
        is_binary=True,
        train_dir=cfg.case_bin_clf.train_dir,
        test_dir=cfg.case_bin_clf.test_dir
    )
    datamodule.prepare_data_info()
    if cfg.case_bin_clf.save_split_result:
        datamodule.save_split_result(
            test_fold=0,
            save_dir=Path(cfg.case_bin_clf.data_dir).parent,
            data_name=cfg.case_bin_clf.data_name
        )
    num_loop = cfg.case_bin_clf.num_split if cfg.case_bin_clf.cross_val else 1
    ...
    for num_fold in range(num_loop):
        ...
        with total_timer:
            if num_loop == 1:
                exp_name = cfg.case_bin_clf.id
            else:
                exp_name = cfg.case_bin_clf.id + f"\\cv{num_fold}"
            datamodule.set_fold(fold=num_fold)
            logger = CSVLogger(save_dir=cfg.case_bin_clf.save_dir, name=exp_name)
            trainer = Trainer(
                accelerator="auto",
                max_epochs=cfg.case_bin_clf.epochs,
            ...
            weight = torch.ones(cfg.case_bin_clf.num_classes)
            if cfg.case_bin_clf.use_weighted_loss:
                weight = datamodule.get_weight()
            model = get_model(
                model_name=cfg.case_bin_clf.model,
                transform=datamodule.test_transform,
                weight=weight,
                **cfg.case_bin_clf,
            )
            if cfg.case_bin_clf.get("checkpoint_path", None) is None:
                with train_timer:
        ...
        visualizer = ResultVisualizer(
            case_id=cfg.case_bin_clf.id,
            image_info_df=datamodule.image_info_df,
            pred_scores=predictions["pred_scores"],
            timer=time_log,
            manual_recall=cfg.case_bin_clf.defect_recall,
            save_dir=Path(logger.log_dir),
        )
        visualizer.save_all()


@hydra.main(version_base=None, config_path="..\\config", config_name="config_bin_clf")
